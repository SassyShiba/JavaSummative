...
public abstract class Property {
  ...
  public void setCouncilTax(double councilTax) {
	...
	int professional_count = 0;

	for (ITenant value : this.rooms.values()) {
	  if (value.getType() == TenantType.PROFESSIONAL) {
		professional_count += 1;
	  }
	}

	if (professional_count == 0 && !this.isAvailable()) {
	  this.councilTax = 0.0;
	} else if (professional_count == 1 && !this.isAvailable()) {
	  this.councilTax *= 0.75;
	}
  }
