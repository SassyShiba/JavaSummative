...
public class PropertyManagement {
  ...
  /**
   * @return percentage of properties that are exempt from council tax.
   */
  public double percentageCouncilTaxExemption() {
	double percentageExemption = 0.0;
	double numberExempt = 0.0;

	for (Property property : this.properties) {
	  if (property.councilTax == 0.0) {
		numberExempt += 1;
	  }
	}
	percentageExemption = numberExempt / this.properties.size();

	return percentageExemption * 100;
  }

  /**
   * @return returns a list of all the properties that are exempt from council
   *         tax.
   */
  public String displayAllCouncilTaxExemptProperties() {
	...
	for (Property property : this.properties) {
	  if (property.councilTax == 0 && !property.isAvailable()) {
		builder.append(property.displayOccupiedProperty()).append("\n");
	  }
	}
	return builder.toString();
  }
